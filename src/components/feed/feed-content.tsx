...
export function FeedContent() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        if (response && (response.posts || response.data?.posts)) {
          let postsData = [];
          
          // Determinar la estructura correcta de la respuesta
          if (response.posts && Array.isArray(response.posts)) {
            // Si la respuesta tiene directamente una propiedad posts
            postsData = response.posts;
          } else if (response.data && response.data.posts && Array.isArray(response.data.posts)) {
            // Si la respuesta tiene una propiedad data que contiene posts
            postsData = response.data.posts;
          }
          
          console.log('Posts procesados:', postsData);
          setPosts(postsData);
        } else {
  ...
  if (loading) {
    return <div className="text-center p-4">Cargando publicaciones...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarImage src={user?.profile_image_url || "https://i.pravatar.cc/150?img=3"} alt="Tu avatar" />
      ...
      // Modificar la parte donde se renderizan los posts
      return (
        <div className="space-y-6">
          {posts.length > 0 ? (
            posts.map((post) => (
              <Card key={post._id || post.id}>
                ...
                <CardFooter className="flex justify-between">
                  <Button 
                    ...
                    onClick={() => console.log(`Like en post ${post._id || post.id}`)}
                  >
